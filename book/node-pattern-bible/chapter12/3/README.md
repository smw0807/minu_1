...
클라이언트와 일련의 원격 API들 간의 통신을 프록시하는 서버인 **API 프록시**(**API 게이트웨이**라고도 함)를 사용하는 것이다.  
마이크로서비스 아키텍처에서 이것의 주된 목적은 여러 API 엔드포인트에 단일 액세스 포인트를 제공하는 것이지만, 로드 밸런싱, 캐싱, 인증 및 트래픽 제한을 제공할 수도 있으며, 이 모든 기능은 견고한 API 솔루션을 구현하는데 매우 유용한 것이다.

이 패턴은 http-proxy 및 consul을 사용하여 사용자정의 로드 밸런서를 만들었을 때 이 장에서 이미 사용 중인 패턴이므로 새로운 패턴은 아니다.  
그 예에서는 로드 밸런서가 두 개의 서비스만 노출했다.  
서비스 레지스트리 덕분에 URL 경로를 서비스와 서버 목록에 매핑할 수 있었다.  
API 프록시는 동일한 방식으로 작동한다.  
본질적으로 역방향 프록시이며 종종 API 요청을 처리하도록 특별히 구성된 로드 밸런서이기도 하다.

![전자상거래 애플리케이션에 API 프록시 패턴을 사용한 예](https://prod-files-secure.s3.us-west-2.amazonaws.com/bc261f43-de91-483d-8946-ac5a65106576/5dcfb5dc-a905-4539-b542-ab6ab84c9037/Untitled.png)

전자상거래 애플리케이션에 API 프록시 패턴을 사용한 예

위 다이어그램에서 API 프록시가 기본 인프라의 복잡성을 어떻게 숨길 수 있는지 명확히 보여야 한다.  
이는 특히 각 서비스가 여러 시스템에 걸쳐 확장된 경우, 노드 수가 많을 수 있는 마이크로서비스 인프라에서 매우 편리하다.  
따라서 API 프록시에 의해 달성된 통합은 의미 메커니즘이 없기 때문에 구조적인 것일 뿐이다.  
복잡한 마이크로서비스 인프라에 대한 접근이 편리한 단일 뷰를 제공하는 것이다.

API 프록시 패턴은 본질적으로 시스템에 존재하는 서로 다른 모든 API에 대한 연결의 복잡성을 추상화하므로, 다양한 서비스를 자유롭게 재구성할 수 있다.  
요구사항이 변경되면 기존 마이크로서비스를 둘 이상으로 분할하거나 반대로 비즈니스 컨텍스트에서 둘 이상의 서비스를 함께 연결할 수도 있다.  
두 경우 모두 API 프록시 패턴을 사용하면 데이터에 접근하는 업스트림 시스템에 영향을 미치지 않고 필요한 모든 변경을 수행할 수 있다.

### API 오케스트레이션

API 오케스트레이션 패턴은 서비스 집합을 통합하고 구성하는 가장 자연스럽고 명시적인 방법이다.  
넷플릭스 API 엔지니어링 부사장인 Daniel Jacobson은 블로그 글에서 다음과 같이 API 오케스트레이션을 정의했다.

<aside>
💡 **API 오케스트레이션 계층(OL: Orchestration Layer)**은 일반적으로 모델링된 데이터 요소 및(또는) 기능을 취하여 대상 개발자 또는 애플리케이션을 위한 보다 구체적인 수단을 제공하는 추상화 계층이다.

</aside>

“일반적으로 모델링된 데이터 요소 및(또는) 기능”은 마이크로서비스 아키텍처의 서비스 설명과 완벽하게 일치한다.  
그 개념은 특정 애플리케이션에 특정한 새로운 서비스를 구현하기 위해 이러한 비트들과 조각들을 연결하는 추상화를 만드는 것이다.

![여러 마이크로서비스와 상호작용하기 위한 오케스트레이션 계층의 사용 예](https://prod-files-secure.s3.us-west-2.amazonaws.com/bc261f43-de91-483d-8946-ac5a65106576/7583a812-6904-4894-a69c-abd54da4c244/Untitled.png)

여러 마이크로서비스와 상호작용하기 위한 오케스트레이션 계층의 사용 예

Store frontend 애플리케이션이 오케스트레이션 계층(API Orchestration Layer)을 사용하여 기존 서비스를 구성하고 조정하여 더 복잡하고 구체적인 기능을 만드는 방법을 보여준다.

위 그림에서 completeCheckout()은 고객이 결제가 끝날 때 지불 버튼을 클릭하는 순간 호출되는 세 단계로 구성된 복잡한 연산으로 구성된 것을 보여준다.

1. 먼저 checkoutService/pay를 호출하여 거래를 완료한다.
2. 결제가 완료되면 Cart 서비스에 상품을 구매했으며 장바구니에서 제거할 수 있음을 알려야 한다.  
   cartService/delete를 호출하여 이를 수행한다.
3. 또한 결제가 완료되면 방금 구매한 상품의 재고 여부를 업데이트 한다.  
   이는 productsService/update를 통해 수행된다.

세가지 다른 서비스의 세가지 연산을 가지고, 전체 시스템을 일관된 상태로 유지하기 위해 서비스를 조정할 수 있음을 보여준다.

**API 오케스트레이션 계층**에서 수행하는 또 다른 일반적인 작업은 **데이터 집계**, 즉 서로 다른 서비스의 데이터를 단일 응답으로 결합하는 것이다.  
Cart에 포함된 모든 제품을 나열하려 한다고 가정했을 때,  
오케스트레이션은 Cart 서비스에서 제품 ID 목록을 검색한 다음, 제품 서비스에서 제품에 대한 전체 정보를 검색해야 한다.  
서비스를 결합하고 조정할 수 있는 방법은 무한하지만 기억해야 할 중요한 패턴은 여러 서비스와 특정 애플리케이션 간의 추상화 역할을 하는 오케스트레이션 계층이다.

오케스트레이션 계층은 추가적인 기능 분할을 위한 좋은 후보이다.  
실제, **API 오케스트레이터**라는 이름을 사용하는 전용 독립 서비스로 구현하는 것이 매우 일반적이다.  
이 관행은 마이크로서비스 철학과 완벽하게 일치한다.

![API 오케스트레이터 패턴을 적용한 모습](https://prod-files-secure.s3.us-west-2.amazonaws.com/bc261f43-de91-483d-8946-ac5a65106576/fcee1833-49b8-4295-89a9-712b2da3528d/Untitled.png)

API 오케스트레이터 패턴을 적용한 모습

위 그림처럼 독립형 오케스트레이터를 생성하면 마이크로서비스 인프라의 복잡성에서 클라이언트 애플리케이션을 분리하는데 도움이 될 수 있다.  
이는 API 프록시와 유사하지만 중요한 차이점이 있다.  
오케스트레이터는 다양한 서비스의 의미 통합을 수행하고 단순한 프록시가 아닌 기본 서비스에 의해 노출된 것과 다른 API를 노출하기도 한다.

### 메시지 브로커와의 통합

오케스트레이터 패턴은 다양한 서비스를 명시적으로 통합하는 메커니즘을 제공했다.  
여기에는 장단점이 있다.  
설계, 디버그 및 확장이 쉽지만 안타깝게도 기본 아키텍처와 각 서비스의 작동 방식에 대한 완전한 지식이 있어야 한다.  
아키텍처 노드 대신 객체에 대해 이야기한다면, 오케스트레이터는 너무 많은 것을 이해하고 수행하는 **God 객체**라는 안티 패턴이 될 것이다.  
이는 일반적으로 높은 결합, 낮은 응집력 그리고 무엇보다도 중요한 높은 복잡성을 야기한다.

지금 보게될 패턴은 서비스 전반에 걸쳐 전체 시스템의 정보를 동기화하는 책임을 분산시키기 위한 것이다.  
그러나 최종적으로 하고 싶은 마지막 일은 서비스간의 직접적인 관계를 만드는 것이다.  
그렇게 한다면, 노드 간의 상호 연결 수가 증가함에 따라 높은 결합과 시스템의 복잡성은 더욱 증가한다.  
따라서 서비스간의 직접적인 관계를 만들되, 구성된 모든 서비스를 분리된 상태로 유지해야 한다.  
모든 서비스는 시스템에 나머지 서비스들 없이도 혹은 새로운 서비스 및 노드와 결합해도 잘 작동할 수 있어야 한다.

이에 대한 해결책은 메시지 수신자로부터 발신자를 분리할 수 있는 시스템인 메시지 브로커를 사용하여 중앙 집중식의 발행(Publish)/구독(Subscribe) 패턴을 구현하는 것이다.

![메시지 브로커를 사용하여 전자상거래 애플리케이션에서 이벤트 배포](https://prod-files-secure.s3.us-west-2.amazonaws.com/bc261f43-de91-483d-8946-ac5a65106576/2c7ac636-22c9-4ed9-a989-50c1b880d015/Untitled.png)

메시지 브로커를 사용하여 전자상거래 애플리케이션에서 이벤트 배포

위 그림에서 볼 수 있듯이 프론트엔드 애플리케이션인 Checkout 서비스의 클라이언트는 다른 서비스와의 명시적인 통합을 수행할 필요가 없다.  
Checkout 프로세스를 완료하고 고객으로부터 돈을 받기 위해 checkoutService/pay를 호출하기만 하면 된다.  
모든 통합 작업은 백그라운드에서 이루어 진다.

1. Store frontend는 Checkout 서비스에서 checkoutService/pay를 호출한다.
2. 작업이 완료되면 Checkout 서비스가 이벤트를 생성하여 작업의 세부 정보, 즉 cartId 및 방금 구입한 제품 목록을 첨부한다.  
   이벤트가 메시지 브로커에 게시된다.  
   이 시점에서 Checkout 서비스는 메시지를 받을 곳을 알지 못한다.
3. Cart 서비스는 브로커에 가입되어 있으므로 Checout 서비스에서 방금 게시(Publish)한 이벤트를 받게 된다.  
   Cart 서비스는 메시지에 포함된 ID로 식별된 장바구니(카드)를 데이터베이스에서 제거함으로써 반응한다.
4. 제품 서비스도 메시지 브로커에 가입되어 있으므로, 동일한 구매 이벤트를 받는다.  
   그런 다음 이 새로운 정보를 기반으로 데이터베이스를 업데이트하여 메시지에 포함된 제품의 제고를 조정한다.

이 전체 프로세스는 오케스트레이터와 같은 외부 엔티티의 명시적인 개입 없이 발생한다.  
지식을 전파하고 정보를 동기화하는 책임은 서비스 자체에 분산되어 있다.  
각 서비스가 통합의 자체 부분을 담당하기 때문에 전체 시스템의 처리 절차를 이해해야 하는 서비스가 존재하지 않는다.

메시지 브로커는 서비스를 분리하고 상호 작용의 복잡성을 줄이는데 사용되는 필수 요소이다.  
또한 영구 메시지 대기열 및 메시지 순서 보장과 같은 다른 흥미로운 기능을 제공할 수도 있다.
