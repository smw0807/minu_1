...
실제 애플리케이션에서는 고급 캐시 무효화 기술과 저장 메커니즘을 사용해야 할 경우가 있다.  
이는 다음과 같은 이유로 필요하다.

- 캐시된 값이 많으면 많은 메모리를 쉽게 사용할 수 있다.  
  이 경우 **최근 사용(LRU: Least Recently Used)** 또는 **선입선출(FIFO: First In First Out)** 정책을 사용하여 일정한 메모리 사용률을 유지할 수 있다.
- 애플리케이션이 여러 프로세스에 분산된 경우, 캐시를 메모리에 유지하면서 각 서버 인스턴스에서 다른 결과가 생성될 수 있다.  
  이에 대한 해결책은 캐시에 공유 저장소를 사용하는 것이다.  
  또한 캐시가 여러 인스턴스에서 공유되므로 단순한 메모리 내 솔루션 보다 성능이 뛰어나다.  
  인기 있는 캐싱 솔루션에는 Redis() 그리고 Memcached()가 있다.
- 시간 만료와 달리 수동 캐시 무효화(예: 캐시되지 않은 관련 값들이 변경되는 경우)는 수명이 긴 캐시를 활성화하고 동시에 더 최신의 데이터를 제공할 수 있지만 관리하기가 훨씬 더 복잡할 것이다.

<aside>
💡 **수동 캐시 무효화**
수동 캐시 무효화는 데이터의 일관성을 유지하는데 중요하지만, 캐시된 데이터가 많고 빈번하게 변경되는 시스템에서는 관리가 복잡해질 수 있습니다. 이런 경우, 자동 캐시 무효화 메커니즘을 구현하는 것이 더 효과적일 수 있습니다. 자동 캐시 무효화는 데이터가 변경될 때 자동으로 캐시를 갱신하거나 무효화하는 기능을 말합니다

</aside>

Phil Karlton의 유명한 인용문을 되새겨 볼 필요가 있다.  
”컴퓨터 과학에는 캐시 무효화와 이름 짓기라는 두 가지 어려운 점이 존재한다.”
