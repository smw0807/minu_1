...
const lsPlugin = require("./languageServerPlugin");
...
class JavaDebugConfigurationProvider {
    ...
    provideDebugConfigurationsAsync(folder, token) {
        return vscode.window.withProgress({ location: vscode.ProgressLocation.Window }, (p) => {
            return new Promise((resolve, reject) => {
                ...
                lsPlugin.resolveMainClass(folder ? folder.uri : undefined).then((res) => {
                    let cache;
                    cache = {};
            ...
            try {
                ...
                if (config.request === "launch") {
                    ...
                    if (_.isEmpty(config.classPaths) && _.isEmpty(config.modulePaths)) {
                        const result = (yield lsPlugin.resolveClasspath(config.mainClass, config.projectName));
                ...
                const debugServerPort = yield lsPlugin.startDebugSession();
                if (debugServerPort) {
    ...
    resolveLaunchConfig(folder, config) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!config.mainClass || this.isFile(config.mainClass)) {
                ...
                if (currentFile) {
                    const mainEntries = yield lsPlugin.resolveMainMethod(vscode.Uri.file(currentFile));
                    if (mainEntries.length === 1) {
                        return mainEntries[0];
                    }
                    else if (mainEntries.length > 1) {
                        return yield this.showMainClassQuickPick(this.formatMainClassOptions(mainEntries), `Multiple main classes found in the file '${path.basename(currentFile)}', please select one first.`);
                    }
                }
                ...
                return yield this.promptMainClass(folder, hintMessage);
            }
            const containsExternalClasspaths = !_.isEmpty(config.classPaths) || !_.isEmpty(config.modulePaths);
            const validationResponse = yield lsPlugin.validateLaunchConfig(folder, config.mainClass, config.projectName, containsExternalClasspaths);
    ...
    promptMainClass(folder, hintMessage) {
        return __awaiter(this, void 0, void 0, function* () {
            const res = yield lsPlugin.resolveMainClass(folder);
            if (res.length === 0) {
                throw new utility.UserError({
            ...
            const selected = yield this.showMainClassQuickPick(pickItems, hintMessage || "Select main class<project name>");
            if (selected) {
...
function needsBuildWorkspace() {
    const debugSettingsRoot = vscode.workspace.getConfiguration("java.debug.settings");
