...
const ts = require("./typescript-extension");
const LANG_PYTHON = "python";
const LANG_JAVA = "java";
const LANG_TYPESCRIPT = "typescript";
const LANG_TYPESCRIPTREACT = "typescriptreact";
const LANG_JAVASCRIPT = "javascript";
const LANG_JAVASCRIPTREACT = "javascriptreact";
const LANG_TYPESCRIPT_AND_JAVASCRIPT = [LANG_TYPESCRIPT, LANG_TYPESCRIPTREACT, LANG_JAVASCRIPT, LANG_JAVASCRIPTREACT];
var apiInstance;
var seenLanguages = [];
...
function activateLanguages(languageIds) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (intersectLists(newLanguageIds, LANG_TYPESCRIPT_AND_JAVASCRIPT).length > 0
            && intersectLists(seenLanguages, LANG_TYPESCRIPT_AND_JAVASCRIPT).length === 0) {
            if (!config_1.ConfigUtil.checkIfIntelliCodeEnabled(LANG_TYPESCRIPT)) {
                telemetry_1.Instance.sendTelemetryEvent(telemetry_1.TelemetryEventNames.USER_CONFIG_INTELLICODE_TYPESCRIPT_COMPLETIONS_DISABLED);
            }
            else {
                languageTelemetryEvent.addProperty(telemetry_1.TelemetryPropertyNames.LANGUAGE, "typescript/javascript");
                try {
                    ts.activate(apiInstance);
                    languageTelemetryEvent.end(telemetry_1.TelemetryResult.Success);
                }
                catch (e) {
                    vscode.window.showErrorMessage("Sorry, something went wrong activating IntelliCode support for TypeScript/JavaScript. Please check the \"VS IntelliCode\" output window for details.");
                    logger_1.Instance.write(`Error while activating TypeScript/JavaScript: ${JSON.stringify(e)}.`);
                    telemetry_1.Instance.sendFault(telemetry_1.TelemetryEventNames.LANGUAGE_ACTIVATION_FAULT, telemetry_1.FaultType.Error, "Error thrown while trying to activate TypeScript/JavaScript", e);
                    languageTelemetryEvent.end(telemetry_1.TelemetryResult.Failure);
                }
            }
        }
...
exports.activateLanguages = activateLanguages;
...
exports.checkPythonExtensionInstalled = checkPythonExtensionInstalled;
...
exports.checkJavaExtensionInstalled = checkJavaExtensionInstalled;
function resetStateForTest() {
    seenLanguages = [];
}
exports.resetStateForTest = resetStateForTest;
